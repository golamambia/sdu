...
if(!this.project){
  ...
  this.alertController.create({
    message:'Please refresh your address',
     buttons: ['OK']
   }).then(resalert => {
  ...
  getLocation(){
    this.geolocation.getCurrentPosition().then((resp) => {
      ...
      this.nativeGeocoder.reverseGeocode(resp.coords.latitude, resp.coords.longitude, options)
       ...
       this.address2=result[0].subLocality+','+result[0].locality+','+result[0].postalCode+','+result[0].subAdministrativeArea
...
async getLocationRel(){
  ...
  getServertime(){
    ...
    this.http.post(host+'get-server-time', JSON.stringify(data),{ headers: headers })
      .subscribe((res:any) => {
       // console.log(res);       
      if(res.status == true){
        this.end_time=res.response_data;
      }
    });
  }
}
