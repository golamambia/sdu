...
@Component({
  selector: 'app-material-receiving-edit',
  templateUrl: './material-receiving-edit.page.html',
  styleUrls: ['./material-receiving-edit.page.scss'],
})
export class MaterialReceivingEditPage implements OnInit {
...
submitted = false;
 ...
 master_item_stock_id_for_receiving:any=[];
 projects_materials_receiving_id:any=[];
 constructor(private http: HttpClient, public navCtrl: NavController,
   ...
   this.despatch_id = this.route.snapshot.paramMap.get('mid');
   this.material_receiving_referencec_id = this.route.snapshot.paramMap.get('mrid');
 ...
 async reloadDepositData(){
      ...
      //this.submitted = true;
        ...
        this.http.post(host+'get-material-despatch-list-byid', JSON.stringify(data),{ headers: headers })
        .subscribe((res:any) => {
         console.log(res);
         loading.dismiss();
        if(res.status == true){
    	...
    	this.depositData.forEach((value,key) => {
    		this.requiredby_qty[key]=value.quantity;
    		this.status[key]=value.status;
          }
            );

    let crt_date = res.all_material_despatch[0].created_at;
...
qty =0;
  ...
  this.master_item_stock_id_for_despatch[key]=this.depositData[key].master_items_stock_id;
  this.projects_materials_receiving_id[key]=this.depositData[key].projects_materials_receiving_id;
  this.master_item_stock_id_for_receiving[key]=this.depositData[key].pmr_master_items_stock_id;
  }
  });
}
...
this.alertController.create({
  	...
  	var data ={
		...
		"sub_project_id":this.sub_project_idnw,
 		...
 		"projects_materials_receiving_id":this.projects_materials_receiving_id,
 		"master_item_stock_id_for_receiving":this.master_item_stock_id_for_receiving,
          }
           
//console.log(data);
           await loading.present();
       
        this.http.post(host+'post-material-despatch-receiving-update', JSON.stringify(data),{ headers: headers })
        .subscribe((res:any) => {
        console.log(res);
         loading.dismiss();
        if(res.status == true){
          ...
          this.alertController.create({
           message: 'Material Receiving Update Successful',
            buttons: ['OK']
          }).then(resalert => {
